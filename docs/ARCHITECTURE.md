...
One exception lives only in `style.css`: the sign-in wall Studocu shows to readers it flags as bots (`isAntiBotAuthWallEligible` in `#__NEXT_DATA__`, issue #63). It is a Modal with no close button, portaled into `#main-wrapper`, that also scroll-locks `<html>` (and `<body>` on Android) with inline styles. The rules match the open `dialog[class*="AuthWall"]` that lacks the `hideable` class the Modal adds when it can be closed, hide its overlay with `:has()`, and override the scroll lock with `!important`. The overlay is a React portal node, so it is hidden, never removed: removing it makes React throw when it unmounts. `test/style.test.mjs` loads the stylesheet into a fixture of the wall to check which declarations apply. After enough automated loads the live site starts serving this wall to the test browser, which is a convenient way to see it.
